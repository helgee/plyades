...
def elements(vector, mu):
    ...
    v_mag = np.sqrt((v ** 2).sum(axis=1))[:, np.newaxis]
    h = np.cross(r, v)
    h_mag = np.sqrt((h ** 2).sum(axis=1))[:, np.newaxis]
    k = np.array([[0, 0, 1]]).repeat(r.shape[0], axis=0)
    n = np.cross(k, h)
    n_mag = np.sqrt((n ** 2).sum(axis=1))[:, np.newaxis]
    xi = v_mag ** 2 / 2 - mu / r_mag
    e = ((v_mag ** 2 - mu / r_mag) * r - v * np.sum(r * v, axis=1)[:, np.newaxis]) / mu
    ecc = np.sqrt((e ** 2).sum(axis=1))[:, np.newaxis]
    if not (ecc == 1).any():
        sma = - mu / (2 * xi)
        p = sma * (1 - ecc ** 2)
    else:
    ...
    peri = np.arccos(np.sum(n * e, axis=1)[:, np.newaxis] / (ecc * n_mag))
    if (e[:, 2] < 0).any():
        peri[e[:, 2] < 0] = 2 * np.pi - peri[e[:, 2] < 0]
    true_ano = np.arccos(np.sum(e * r, axis=1)[:, np.newaxis] / (ecc * r_mag))
    if (np.sum(r * v, axis=1) < 0).any():
        true_ano[np.sum(r, v, axis=1) < 0] = 2 * np.pi - true_ano[np.sum(r * v, axis=1) < 0]
    if sma.shape == (1, 1):
        print np.hstack([sma, ecc, inc, node, peri, true_ano]).flatten()
